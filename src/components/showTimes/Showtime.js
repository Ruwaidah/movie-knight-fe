...
import { getTheatersInfo } from "../../features/movies/moviesSlice";
...
const Showtime = (props) => {
  ...
  const theatersId = [];
  const { results, theatresInfo, gettingTheaters,movieSelect, allMovies } = useSelector(
    (state) => state.movies
  );
  useEffect(() => {
    console.log("theaters", movieSelect);
    console.log(allMovies)
    results.map((movies) =>
      movies.showtimes.map((thea) => theatersId.push(thea.id))
    );
    dispatch(getTheatersInfo(theatersId));
  ...
  //   ...obj,
  // }));
  console.log(results, theatresInfo);
  // console.log(moviesShow);
  if (gettingTheaters) return <Loading/>
  return (
    <div className="showtime-card">
      <h3 className="text">Your matches</h3>
      {results.map((movieslist, index) => {
                let movie = allMovies.filter(mo => mo.tmsId
                  === movieslist.id)
                if (movie.length > 0)
                    return <div key={movieslist.id}>
                        ...
                        {movieslist.showtimes.map((show, ind) => {
                            return <div key={show.id + movieslist.id} className={index === results.length - 1 ? ind === movieslist.showtimes.length - 1 ? "lasttheater" : null : null}>
